...
public class RegisterUser {
    ...
    //private static final Set<String> VALID_ROLES = Set.of("admin", "backoffice", "partner", "enduser");
    ...
    public Response registerUser(String body) {
        ...
        try {
            ...
            if (user.username == null || user.email == null || user.password == null || user.confirmacao == null) {
         ...
         // Impedir que o cliente defina manualmente 'role' ou 'estado'
            ...
            while (results.hasNext()) {
                ...
                if (!e.getKey().getName().equals("root")) {
                    count++;
                }
            }
            if (count >= 4) {
                return Response.status(400).entity("{\"erro\": \"Limite de utilizadores atingido\"}").build();
            }
         ...
         // Verificar se já existe utilizador com o mesmo email
            ...
            QueryResults<Entity> telefoneResults = datastore.run(telefoneQuery);
            if (telefoneResults.hasNext()) {
